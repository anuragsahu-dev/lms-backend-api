...
const userSchema = new mongoose.Schema(
    {
        ...
        password: {
            ...
            // bcrypt produces 60-char hashes, so max must be higher
            // than 60 to allow the hashed password to be stored
            maxLength: [128, "Password length not more than 128 characters"],
            select: false,
        },
        role: {
            type: String,
...
userSchema.index({ email: 1 }, { unique: true }); // point 1
...
userSchema.methods.getAccessToken = function () {
    return jwt.sign(
        ...
        { expiresIn: process.env.ACCESS_TOKEN_EXPIRE || "4h" }
    );
};
